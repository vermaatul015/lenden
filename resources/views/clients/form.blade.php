...
<div class="radio">
    <label><input name="is_active" type="radio" value="{{ $optionKey }}" @if($i == 0) required @endif @if (isset($data['client'])) {{ (isset($data['client']->is_active) && $data['client']->is_active == $optionKey) ? 'checked' : ''}}  @else {{ 'checked' }} @endif> @lang('clients.is_active_'.strtolower(str_replace(" ","_",$optionValue)))</label>
</div>
@php
$i++;
@endphp
